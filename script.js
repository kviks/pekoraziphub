Promise.all([
  fetch('https://raw.githubusercontent.com/kviks/items/refs/heads/main/items.json').then(res => res.json()),
  ...
  items.forEach(item => {
    ...
    <h2>${item.name}</h2>
    ${valueEntry ? `
      <div class="value-badge">V: ${valueEntry.Value}</div>
      <div class="demand-badge">D: ${valueEntry.Demand}</div>
      ${valueEntry.IsRare ? `<div class="rare-badge">RARE</div>` : ''}
    ` : ''}
